...
# 视频处理滤镜链（优化人物细节和兼容性）
# 超保守的降噪参数，最大化保留人物面部纹理、皮肤细节和毛发
# 极轻微的锐化，保持自然效果，避免过度锐化
# 非常细微的色彩调整，提升细节可见性同时保持与原视频一致
FILTER_CHAIN="fps=$FPS,hqdn3d=0.1:0.05:0.3:0.2,unsharp=3:3:0.1:3:3:0.05,eq=contrast=1.005:brightness=0.01:saturation=1.00,scale=${WIDTH}:-1:flags=$INTERPOLATION,select='lt(n,$MAX_FRAMES)',setpts=N/$FPS/TB"

# 核心转换命令（优化人物细节编码参数）
echo -e "${YELLOW}正在生成高保真WebP动图...${NC}"
ffmpeg -y \
